...
struct kvm_gmem {
	...
	struct guest_memfd gmem;
};

static inline struct kvm_gmem *guest_memfd_to_kvm_gmem(struct guest_memfd *gmem)
{
	return container_of(gmem, struct kvm_gmem, gmem);
}

static inline struct kvm_gmem *inode_to_kvm_gmem(struct inode *inode)
{
	return guest_memfd_to_kvm_gmem(inode->i_private);
}

static inline struct kvm_gmem *file_to_kvm_gmem(struct file *file)
{
	if (!is_guest_memfd(file))
		return NULL;

	return guest_memfd_to_kvm_gmem(file->private_data);
}
...
static int __kvm_gmem_create(struct kvm *kvm, loff_t size, u64 flags)
{
	...
	gmem->gmem.ops = &kvm_gmem_ops;
	file = guest_memfd_alloc(anon_name, &gmem->gmem, size);
	if (IS_ERR(file)) {
		err = PTR_ERR(file);
		goto err_gmem;
	}
		       ...
		       gfn_t gfn, kvm_pfn_t *pfn, int *max_order, bool prepare)
{
	...
	folio = guest_memfd_grab_folio(file_inode(file), index, flags);
	if (IS_ERR(folio))
